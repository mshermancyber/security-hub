...
export function SectorPanel({ onSelectSector }: SectorPanelProps = {}) {
  ...
  const visible = showEmpty ? sectors : sectors.filter(s => s.score > 0 || s.mentions > 0)
  const max = Math.max(1, ...sectors.map(s => s.score))
    ...
    <Panel code="SECTOR" title="Sector heatmap (30d targeting signal)" hotkey="6"
      ...
      {loading ? <div className="p-3 text-mute text-xs">loading...</div> : visible.length === 0 ? (
        ...
        <ul className="text-[12px]">
          {visible.map(s => {
            const w = Math.round((s.score / max) * 100)
            ...
            return (
              <li key={s.id}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-mute tabular-nums w-10 text-right">{s.score}</span>
                  <span className="text-mute/50 tabular-nums w-8 text-right text-[10px]">{s.mentions}m</span>
                </div>
