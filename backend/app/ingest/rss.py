...
async def fetch_feed(feed_cfg: dict) -> int:
    ...
    # Pre-screen for billion-laughs / XXE-style payloads before handing to
    # feedparser. feedparser 6.x disables external-entity resolution but
    # the SAX parser still expands internal entities, which is the
    # exploitable vector for recursive expansion attacks. Check case-
    # insensitively over the prolog only: XML/HTML parsers in error-recovery
    # mode accept mixed-case `<!entity`/`<!doctype`.
