    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30, headers=headers,
            ...
            for conf in USENIX_RECENT_CONFS:
                year_match = re.search(r"(\d{2,4})$", conf)
                conf_year = int(year_match.group(1)) if year_match else datetime.now(timezone.utc).year
                if conf_year < 100:
                    conf_year += 2000
                published = datetime(conf_year, 8, 1, tzinfo=timezone.utc).isoformat()
                try:
