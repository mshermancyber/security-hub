...
def _select_candidates(limit: int = 40) -> list[str]:
    """Return CVE IDs worth fetching from CVE.org. Only recent CVEs —
    ADP refreshes on old CVEs are metadata churn, not actionable news.
    """
    rows = fetchall(
        """
        SELECT cve_id FROM cves
         WHERE is_kev = 1
           AND datetime(published_at) >= datetime('now', '-10 days')
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20, headers=headers,
            ...
            for cve_id in cve_ids:
                ...
                url = f"https://www.cve.org/CVERecord?id={quote(cve_id)}"
                persist_news(
