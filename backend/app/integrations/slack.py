...
def _format(event: dict) -> dict:
    ...
    if et == "news.item":
        ...
        return {
            "text": f"[P{prio}] {it.get('title','(no title)')}",
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn",
                 "text": f"*[P{prio}] {it.get('title')}*\n_{it.get('source_name')}_"}},
                {"type": "context", "elements": [{"type": "mrkdwn",
                 "text": f"tags: `{', '.join(it.get('tags') or [])}`  ·  <{it.get('url')}|open>"}]},
            ],
        }
    if et == "org.alert":
        it = event.get("item", {})
        return {
            "text": f"[ORG:{event.get('org')}] {it.get('title','')}",
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn",
                 "text": f"*🚨 Org alert: {event.get('org_name', event.get('org'))}*\n{it.get('title')}"}},
                {"type": "context", "elements": [{"type": "mrkdwn",
                 "text": f"reason: `{event.get('reason')}`  ·  {it.get('source_name')}  ·  <{it.get('url')}|open>"}]},
            ],
        }
    if et == "kev.batch":
        top = event.get("top") or []
        lines = [f"• `{c.get('cve_id')}` — {(c.get('description') or '')[:80]}" for c in top]
        return {
