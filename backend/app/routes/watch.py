...
def _sector_score(mentions: int, tags: dict[str, int]) -> int:
    """Weighted threat score: breach/ransomware dominate, general mentions are low signal."""
    s = (
        tags.get("breach", 0) * 25
        + tags.get("ransomware", 0) * 20
        + tags.get("active-exploitation", 0) * 15
        + tags.get("zero-day", 0) * 15
        + tags.get("vulnerability", 0) * 3
        + tags.get("patch", 0) * 2
        + max(0, mentions - sum(tags.get(t, 0) for t in
              ("breach", "ransomware", "active-exploitation", "zero-day", "vulnerability", "patch")))
    )
    return min(s, 100)
...
def sector_heatmap(days: int = Query(30, ge=1, le=365)):
    ...
    for s in sectors:
        ...
        score = _sector_score(count, tags)
        out.append({
            "id": s["id"],
            "name": s["name"],
            "score": score,
