...
> **v1.0.0 status update**: this file is the historical audit log for
> Rounds 1–3. Rounds 4–6 (security hardening, mobile redirect, release
> prep) shipped further fixes beyond what's recorded below. As of
> v1.0.0, additionally **SHIPPED** and not yet reflected in the
> per-finding tables: M-2 (LLM prompt-injection: `_sanitize_source_text`
> fence + invisible-unicode strip), L-11 (workspace rate-limiting via
> `app/ratelimit.py`), L-15 (`SECHUB_METRICS_TOKEN` gates `/metrics`),
> plus full SSRF/body-cap/redirect-guard on every outbound HTTP call
> (`app/safe_http.py`), token-gated WebSocket (`main.py`), mobile-UA
> redirect (`mobile_redirect.py`), container hardening (`cap_drop: ALL`,
> `read_only`, non-root uid 10001). Remaining genuinely open: M-3
> (LIKE perf — design-level), M-4 (Postgres adapter — design-level),
> L-5/L-10/L-12/L-14 (perf / robustness, not security).
